...
# ╔═╡ e47977be-94d9-11ec-3522-3306c97ebb41
begin
	using CSV
	using DataFrames
	using Tables
...
# ╔═╡ 745ebf9c-17dd-49e2-8d0f-cb4a7ba81d45
begin
	TEST_SIZE = 0.20
	LEARNING_RATE = 0.01
	EPOCHS = 20
...
# ╔═╡ b6cc02a7-bde1-4f14-918e-2e12950dbc97
df = DataFrame(CSV.File("./MINST.csv"))

# ╔═╡ d87d86ec-7962-4266-b641-fc3b2d7f3aeb
begin
	x = transpose(Tables.matrix(select(df, Not(:label))))
	x = Float64.(x)
end

# ╔═╡ ae5b955c-408b-4ee8-be31-672ffce500d3
begin
	y = unique(df.label) .== permutedims(df.label)	
	y = Float64.(y)
end

# ╔═╡ f883f8b9-75db-4004-9501-f151e675ad50
(train_x, train_y), (test_x, test_y) = splitobs((x, y), at=1-TEST_SIZE)

# ╔═╡ 0d6b9aab-1d7c-4a3c-87f6-c41f0b07d05d
m3 = Chain(
	Dense(784, 256, relu),
	Dense(256, 10, relu),
	softmax
...
# ╔═╡ 53cd3b1e-88c0-449d-a495-1c9b23c6481f
m4 = Chain(
	Dense(784, 512, relu),
	Dense(512, 256, relu),
	Dense(256, 10, relu),
...
function train(m, train_x, train_y, test_x, test_y)
	train_loader = DataLoader((train_x, train_y), batchsize=100, shuffle=true)
...
model3, trainm3, testm3 = train(m3, train_x, train_y, test_x, test_y)
...
categorical_accuracy(model4(test_x), test_y)

# ╔═╡ 0605add6-e65b-45d7-86ab-99cf1ee7e90b
begin
	plot(
		1:size(trainm3)[1], [trainm3, trainm4],
		title = "Training Accuracy",
		label = ["3-layer" "4-layer"],
		xlabel = "Epochs",
		ylabel = "Accuracy (%)",
	)
end

# ╔═╡ b22b10a0-2fe2-487c-b12c-636342163213
begin
	plot(
		1:size(trainm3)[1], [testm3, testm4],
		title = "Test Accuracy",
		label = ["3-layer" "4-layer"],
		xlabel = "Epochs",
		ylabel = "Accuracy (%)",
